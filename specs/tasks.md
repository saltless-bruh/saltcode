...
- [ ] 3.2 `scope_probe.py`: if `--scope` absent, one `outline` MCP call → sorted module/file list (a tool call, NOT a Phase-1 fire). Entrypoint `saltcode.tools.scope_probe`.
  > **BLOCKED 2026-07-28 — spec defect, awaiting the maintainer's decision.** `outline(file_path: str) -> str` (already implemented, `saltcode/mcp/lsp_ast_server.py:63`) takes **one file** and returns **that file's symbols**. No single `outline` call can produce a repo-wide "sorted module/file list", so this step and REQ-CACHE-002's lookup-time clause cannot both be read literally. Two readings give different code *and* different cache keys, so per `specs/requirements.md` ("Ambiguity is a defect… the requirement — not the code — is fixed first") the requirement is amended before the code is written. Separately, the existing `run_scope_probe(goal, workspace)` matches **neither** reading: it token-matches filenames out of the goal string and never calls `outline` at all — a pre-existing defect against 3.2 regardless of how the ambiguity resolves.
...
- **Verification (2026-07-28, backend `bwrap`) — 4 of 5 legs PASSED; the task is NOT done.** 164 tests green (114 pre-existing + 50 new in `tests/test_task_3_containment.py`, 0 skipped on this host); `ruff` clean; `pyright --strict` 0 errors. Legs 1–4 are covered: contained apply + no network (`test_no_network_inside_the_container` — a real `socket.create_connection` refused) + read-only host fs (`test_the_host_filesystem_is_read_only_outside_the_sandbox`); `rm -rf ~/` executed through the real test runner leaves both host canaries and the host `$HOME` intact (`test_rm_rf_home_does_not_affect_the_host`); refusal + logging (`test_a_refused_command_is_logged_as_refused`); refusal to run Phase 2 with no backend (`test_no_backend_refuses_phase_2`). **Leg 5 (the scope probe via subprocess) is UNVERIFIED** because 3.2 is blocked above — `saltcode.tools.scope_probe` is deliberately not written yet.

...
- [ ] 4.6 **Consumption path:** author `mcp.json` declaring the server over stdio (`{ "command":"python", "args":["-m","saltcode.mcp.lsp_ast_server"], "transport":"stdio", "lifecycle":"eager" }`); consume via an MCP client extension (`pi-mcp-extension`) — do NOT hand-roll a bridge. Verify tools register as `mcp_saltcode-lsp_*` and that the extension's `tool_call` handler still gates them (scope block on out-of-range `read_file`).
  > **PARTIAL — `mcp.json` authored 2026-07-28; adoption BLOCKED on a maintainer decision.** Two halves. (a) `mcp.json` exists at the repo root, declaring `saltcode-lsp` over stdio in the shape this step specifies; it is marked in-file as **unvalidated** against any client extension's schema. (b) Adoption cannot proceed here for two reasons. **A trust decision:** `pi-mcp-extension` (v1.5.0, MIT, last published 2026-05-03) and the more actively maintained `pi-mcp-adapter` (v2.15.0, MIT, 2026-07-25) both exist on npm; installing either runs third-party code with full system permissions, which REQ-SEC-006 AC2 and REQ-EXT-015 AC4 require be trust-reviewed **before** install — not a call to make unilaterally. **An ordering conflict:** this step also says to verify the bridged tools are gated by "the extension's `tool_call` handler", but that handler is **Task 13.3** and `extensions/saltcode.ts` is still the Task-0 stub. Task 20.1/20.3 (`deps: 4, 7, 13`) exist precisely to wire and prove this, so the verification half belongs there. The backend side is finished and proven: the server runs as a real MCP server over stdio and registers exactly `find_references`, `outline`, `where_is` (`test_the_server_registers_exactly_the_three_ast_tools`).

...
- **At lookup time** (before Phase 1): from (a) an explicit `--scope` argument, or (b) `saltcode_scope_probe` — a single `outline` MCP call producing a sorted module/file list. This probe is a tool call, NOT a Phase-1 fire.
